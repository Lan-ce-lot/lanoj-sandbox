...
int _c_cpp_seccomp_rules(struct execConfig *_config, bool allow_write_file) {
    ...
    if (seccomp_rule_add(ctx, SCMP_ACT_ALLOW, SCMP_SYS(execve), 1, SCMP_A0(SCMP_CMP_EQ, (scmp_datum_t)(_config->execPath))) != 0) {
        return LOAD_SECCOMP_FAILED;
    }
    if (!allow_write_file) {
...
int c_cpp_seccomp_rules(struct execConfig *_config, bool allow_write_file) {
